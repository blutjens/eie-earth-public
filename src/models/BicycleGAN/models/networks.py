...
def define_G(input_nc, output_nc, nz, ngf, netG='unet_128', norm='batch', nl='relu',
    ...
    if netG == 'unet_128' and where_add == 'input':
        ...
        net = G_Unet_add_all(input_nc, output_nc, nz, 8, ngf, norm_layer=norm_layer, nl_layer=nl_layer,
                             use_dropout=use_dropout, upsample=upsample)
    elif netG == 'unet_1024' and where_add == 'input':
        net = G_Unet_add_input(input_nc, output_nc, nz, 10, ngf, norm_layer=norm_layer, nl_layer=nl_layer,
                               use_dropout=use_dropout, upsample=upsample)
    elif netG == 'unet_1024' and where_add == 'all':
        net = G_Unet_add_all(input_nc, output_nc, nz, 10, ngf, norm_layer=norm_layer, nl_layer=nl_layer,
                             use_dropout=use_dropout, upsample=upsample)
    else:
...
def define_D(input_nc, ndf, netD, norm='batch', nl='lrelu', init_type='xavier', init_gain=0.02, num_Ds=1, gpu_ids=[]):
    ...
    print('+++++++++++ defining D with netD: ',netD)
    if netD == 'basic_128':
    ...
    elif netD == 'basic_1024_multi':
        net = D_NLayersMulti(input_nc=input_nc, ndf=ndf, n_layers=5, norm_layer=norm_layer, num_D=num_Ds)
    else:
...
def define_E(input_nc, output_nc, ndf, netE,
    ...
    if netE == 'resnet_128':
        ...
        net = E_ResNet(input_nc, output_nc, ndf, n_blocks=7, norm_layer=norm_layer,
                       nl_layer=nl_layer, vaeLike=vaeLike)
    elif netE == 'conv_128':
...
class E_ResNet(nn.Module):
    def __init__(self, input_nc=3, output_nc=1, ndf=64, n_blocks=4,
        ...
        max_ndf = 4
        print('+++++++++ E_ResNet: input and output channels, ndf ',input_nc, output_nc, ndf)
        conv_layers = [
